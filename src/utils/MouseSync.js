...
function MouseSync(options) {
    ...
    this._eventInput.on('mouseup', _handleEnd.bind(this, false));
    if (this.options.crossover) this._eventInput.on('mouseenter', _handleCrossover.bind(this));

    //if (this.options.propogate) this._eventInput.on('mouseleave', _handleLeave.bind(this));
    this._eventInput.on('mouseleave', _handleEnd.bind(this, true));
...
function _handleCrossover(event) {
    if (Date.now() - this._crossoverTimestamp < 10) {
...
MouseSync.prototype.setCrossoverTimestamp = function(timestamp) {
    this._crossoverTimestamp = timestamp;
};
...
function _handleEnd(isMouseleave, event) {
    if (this.options.snapOnLeave) this._payload.snapOnLeave = event.offsetY;
    if (!this._down) return;
    if (isMouseleave) this._payload.timestamp = Date.now();
...
MouseSync.prototype.setOptions = function setOptions(options) {
    ...
    if (options.crossover !== undefined) this.options.crossover = options.crossover;
    if (options.snapOnLeave !== undefined) this.options.snapOnLeave = options.snapOnLeave;
};
