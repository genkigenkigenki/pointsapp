...
ScrollSync.prototype.newFrameLoop = _newFrame;
...
function _newFrame() {
    var endLoop = this._inProgress && (_now() - this._prevTime) > this.options.stallTime;
    if (endLoop) {
        this._inProgress = false;
...
function _handleMove(event) {
    ...
    if (!this._inProgress) {
        ...
        if (!this._loopBound) {
            this._eventOutput.emit('frameLoop');
    ...
    var deltaMultiplier = 0;
    var absolutePos = Math.abs(this._position);
    if (absolutePos <= 100) deltaMultiplier = 0.5;
    else if (absolutePos <= 200) deltaMultiplier = 1;
    else if (absolutePos <= 600) deltaMultiplier = 2;
    else deltaMultiplier = 3;
    diffX = diffX * deltaMultiplier;
    diffY = diffY * deltaMultiplier;
    ...
    payload = this._payload;
    if (( payload.delta > 0 ) !== ( nextDelta > 0 )) {
        this._eventOutput.emit('start', payload);
    }
